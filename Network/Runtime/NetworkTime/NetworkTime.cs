...
namespace RiseOn.Utils.Network {
    ...
    /// Asks NTP servers first, then falls back to the Date header of a few HTTPS endpoints, and retries until the time budget runs out.<br/>
    /// On WebGL, which has neither, it reads the time from HTTPS endpoints the browser is allowed to read.
    /// </summary>
    public static class NetworkTime {
        ...
        private const int   BrowserAttemptTimeoutMilliseconds = 1000;
        ...
        // A browser has no UDP for NTP and hides the Date header of other sites, so WebGL reads the time from the body of
        // endpoints that let any page read them (CORS).
        private static readonly Uri[] browserTimeUris = {
            new("https://www.cloudflare.com/cdn-cgi/trace")
        ...
        /// On WebGL it asks HTTPS endpoints through the browser instead of NTP; call it from the main thread there.<br/>
        /// Throws <see cref="TimeoutException"/>, listing every failed attempt, when nothing answers within <paramref name="timeoutSeconds"/>.
        /// </summary>
        public static Task<DateTime> NowAsync(
          ...
          , CancellationToken cancellationToken = default) {
            ...
            return TryUntilTimeout(Application.platform is RuntimePlatform.WebGLPlayer, timeoutSeconds, cancellationToken);

            // WebGL has no threads, so there the awaits stay on Unity's context and nothing waits on a timer
            // (no NTP timeouts, no Task.Delay between rounds); every browser attempt already waits at least a frame.
            static async Task<DateTime> TryUntilTimeout(bool inBrowser, float timeoutSeconds, CancellationToken cancellationToken) {
                ...
                while (stopwatch.Elapsed.TotalSeconds < timeoutSeconds) {
                    ...
                    foreach (var uri in inBrowser ? browserTimeUris : httpTimeUris) {
                        cancellationToken.ThrowIfCancellationRequested();
                        var timeoutMilliseconds = GetRemainingTimeoutMilliseconds(
                            stopwatch, timeoutSeconds, inBrowser ? BrowserAttemptTimeoutMilliseconds : HttpAttemptTimeoutMilliseconds);
                        if (timeoutMilliseconds <= 0) break;

                        ++attempt;
                        try {
                            return inBrowser
                                ? await GetBrowserTime(uri, timeoutMilliseconds, cancellationToken)
                                : await GetHttpDateTime(uri, timeoutMilliseconds, cancellationToken).ConfigureAwait(false);
                        } catch (OperationCanceledException) when (cancellationToken.IsCancellationRequested) {
                            throw;
                        } catch (Exception ex) {
                            failures.Add(new Exception($"Attempt {attempt} via HTTPS '{uri}' failed: {ex.Message}", ex));
                        }
                    }
            ...
            static async Task<DateTime> GetNtpTime(
                string ntpServer
              , Stopwatch stopwatch
              , float timeoutSeconds) {
                ...
                using (var socket = new Socket(ipEndPoint.AddressFamily, SocketType.Dgram, ProtocolType.Udp)) {
                    ...
                    timeoutMilliseconds = GetRemainingAttemptTimeoutMilliseconds(
                        stopwatch, attemptStopwatch, timeoutSeconds, NtpAttemptTimeoutMilliseconds);
                    var receivedBytes = await ResultWithTimeout(
            ...
            static async Task<IPEndPoint> GetNtpEndPoint(
                string ntpServer
              , Stopwatch stopwatch
              ...
              , float timeoutSeconds) {
                ...
                var timeoutMilliseconds = GetRemainingAttemptTimeoutMilliseconds(
                    stopwatch, attemptStopwatch, timeoutSeconds, NtpAttemptTimeoutMilliseconds);
                var addresses = await ResultWithTimeout(
            ...
            static async Task<DateTime> GetHttpDateTime(
                Uri uri
            ...
            static async Task<DateTime> GetBrowserTime(
                Uri uri
              , int timeoutMilliseconds
              , CancellationToken cancellationToken) {
                var attemptStopwatch = Stopwatch.StartNew();
                var body             = await BrowserRequest.GetText(uri, timeoutMilliseconds / 1000f, cancellationToken);

                // The server read its clock somewhere during the round trip; halfway is the best guess.
                return ReadBrowserTime(uri, body).AddTicks(attemptStopwatch.Elapsed.Ticks / 2);
            }

            // Cloudflare's trace is one key=value per line, ts being Unix seconds with a fraction.
            // timeapi.io answers JSON whose dateTime is the UTC wall clock with no offset, as asked for in the query.
            static DateTime ReadBrowserTime(Uri uri, string body) {
                foreach (var line in body.Split('\n')) {
                    if (!line.StartsWith("ts=", StringComparison.Ordinal)) continue;

                    var unixSeconds = double.Parse(line.Substring(3).Trim(), NumberStyles.Float, CultureInfo.InvariantCulture);
                    return DateTimeOffset.FromUnixTimeMilliseconds((long)Math.Round(unixSeconds * 1000d)).UtcDateTime;
                }

                var dateTime = Regex.Match(body, "\"dateTime\"\\s*:\\s*\"([^\"]+)\"");
                if (dateTime.Success) {
                ...
                throw new FormatException($"Response from '{uri}' has neither a 'ts=' line nor a 'dateTime' field.");
            }

            static int GetRemainingTimeoutMilliseconds(
                Stopwatch stopwatch
              , float timeoutSeconds
            ...
            static int GetRemainingAttemptTimeoutMilliseconds(
                Stopwatch stopwatch
              , Stopwatch attemptStopwatch
              , float timeoutSeconds
            ...
            static async Task WithTimeout(Task task, int timeoutMilliseconds, string timeoutMessage) {
            ...
            static async Task<T> ResultWithTimeout<T>(Task<T> task, int timeoutMilliseconds, string timeoutMessage) {
                if (timeoutMilliseconds <= 0) throw new TimeoutException(timeoutMessage);
            ...
            // A socket call left behind by a timeout fails once the socket is disposed.
            // Reading its exception keeps it from being reported as an unobserved task exception.
            static void ObserveLateFault(Task task) {
                task.ContinueWith(
            ...
            static string BuildFailureSummary(IReadOnlyList<Exception> failures) {
